...
const addApplication = async (req, res) => {
  ...
  try {
    const applicationId = await applicationservice.addApplication(
      courseData,
      userId,
      userRole
    );
...
const uploadDocuments = async (req, res) => {
  ...
  if (!requiredFields.every((field) => files[field])) {
    return res
      ...
      .json({
        error:
          "Please provide all required files (aadhar, pan, pass_front, and pass_back).",
      });
  }
  ...
  try {
    ...
    if (applications.length === 0) {
      const noApplicationsMessage =
        "No applications found for the given criteria.";
      res.status(200).json({
        message: noApplicationsMessage,
      });
    } else {
      const studentFound = applications.some(
        (application) => application.studentName === studentName
      );
      if (studentFound) {
...
//         case 'admin':
//           const allApplications = await applicationservice.getallapplication(offset, pageSize);
...
//         case 'staff':
//           const allApplicationsByCountry = await applicationservice.getApplicationsByAdminCountry(userId,offset, pageSize);
...
//             responseMessage = 'No user applications found for the provided user ID.';
//           }
//           const totalCount = await applicationservice.getTotalApplicationCount();
...
//             await applicationservice.getUserApplicationByPhoneNumber11(
//               userId,
//               applicationStatus
//             );
...
//               await applicationservice.getUserApplicationByPhoneNumber(
//                 userId,
//                 searchKey
//               );
...
//             } else {
//               responseMessage =
//                 "No user application found for the provided phone number.";
//             }
//           } else {
//             const userApplications =
//               await applicationservice.getUserApplicationsByName(
//                 userId,
//                 searchKey
//               );
...
//         } else {
//           const userApplications =
//             await applicationservice.getAllUserApplications(userId, userRole);
...
//           } else {
//             responseMessage =
//               "No user applications found for the provided user ID.";
//           }
//         }
...
//         if (allstu.length > 0) {
//           responseMessage =
//             "All student applications data fetched successfully";
//           responseData = allstu;
//         } else {
//           responseMessage =
//             "No user applications found for the provided user ID.";
...
//       case "staff":
//         const allApplicationsByCountry =
//           await applicationservice.getApplicationsByAdminCountry(
//             userId,
//             offset,
//             pageSize
//           );
...
//         if (allApplicationsByCountry.length > 0) {
//           responseMessage = "All staff applications fetched by Country";
//           responseData = allApplicationsByCountry;
//         } else {
//           responseMessage =
//             "No user applications found for the provided user ID.";
//         }
...
//       default:
//         res.status(400).json({ status:400, message: "Invalid user role" });
//         return;
//     }
  ...
  try {
    ...
    switch (userRole) {
      ...
      case 'admin':
        ...
        if (allApplications) {
          totalCount = await applicationservice.getTotalApplicationCount();  // Include totalCount here
          const totalPages = Math.ceil(totalCount / pageSize);
      ...
      case 'student':
        ...
        if (allstu.length > 0) {
          totalCount = await applicationservice.getTotalApplicationCount();  
          const totalPages = Math.ceil(totalCount / pageSize);
      ...
      case 'staff':
        ...
        if (allApplicationsByCountry.length > 0) {
          totalCount = await applicationservice.getTotalstaffdata(userId); 
          const totalPages = Math.ceil(totalCount / pageSize);

          responseMessage = 'All staff applications fetched by Country';
          responseData = {
            applications: allApplicationsByCountry,
            pagination: {
              page: parseInt(page),
              pageSize: parseInt(pageSize),
              totalItems: totalCount,
...
const notifystatus = async (req, res) => {
  ...
  try {
    if (userId) {
      ...
      res.status(201).json({
        message:
          "User application information and comments  retrieved successfully",
        status: 201,
    ...
    const errorMessage =
      "Error fetching user application information: " + error.message;
    res.status(500).json({ error: errorMessage });
  }
};
...
const getexcelshheetdata = async (req, res) => {
  ...
  try {
    ...
    if (userRole === "admin") {
      excelFilePath = await applicationservice.getExcelDataForAllApplications(
        userRole
      );
...
const getcooment = (req, res) => {
  ...
  const userRole = req.user.role;
  applicationservice
    .getcomment(userId, applicationId, comment_text, userRole, select_type)
    .then((insertResult) => {
...
const updateDocuments = async (req, res) => {
  ...
  if (files["aadhar"]) {
    ...
    updates.push(
      applicationservice.updateApplicationDocument(
        userId,
        "aadhar_card",
        aadharFileName
      )
    ...
    res
      .status(200)
      .json({ status: 201, message: "aadhar document updated successfully" });
    return;
  }
  ...
  if (files["pan"]) {
    ...
    res
      .status(200)
      .json({ status: 201, message: "Pan card updated successfully" });
    return;
  }
  ...
  if (files["pass_front"]) {
    ...
    res
      ...
      .json({
        status: 201,
        message: "Passport front document updated successfully",
      });
    return;
  }
  ...
  if (files["pass_back"]) {
    ...
    res
      ...
      .json({
        status: 201,
        message: "Passport Back document updated successfully",
      });
    return;
  }
  ...
  if (files["10th"]) {
    ...
    res
      .status(200)
      .json({ status: 201, message: "10th document updated successfully" });
    return;
  }
